...
class PipelineRunner(object):
    ...
    def kill_all(self):
        ...
        # NB: the kill_one method will deliberately kill one run
        # from the pipeline. 

    def restart_nprocs(self, pipe_id, params):
        """Kill one running component and restart it with new arguments"""
        status = 1
        _log.warn("killing  and adding a new task...\n")
        still_running = {}
        print("Inside kill_and_add")
        with self.pipelines_lock:
            self._killed = True
            self._allow_new_pipelines = False
            self._process_pipelines = False
        ...
        for pipe in still_running:
            if pipe.id == pipe_id: 
                ...
                #with self.free_cv:
                    ...
                    #else:
                    #    _log.error(
                    #         "pipeline '%s' requires %d nodes after addition > free %d, skipping",
                    #          pipe.id, pipe.get_nodes_used() + 1, self.free_nodes)
                    #    break
                print("Killing the run" + params["run-name"])
                #pipe.force_restart(params)    
                #test if a new process can be spawned..accordingly return success or failure
                restart_thread = threading.Thread(target=pipe.force_restart(self, params))
                restart_thread.start()

                #while restart_thread.isAlive() == False:
                #    continue
