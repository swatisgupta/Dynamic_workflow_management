...
CONNECTION_NAME = 'workflow.connect'
...
class Run(threading.Thread):
    ...
    def __init__(self, name, exe, args, env, working_dir, timeout=None,
                 ...
                 return_path=None, walltime_path=None, app_path=None,
        ...
        self._deliberatily_killed = False #To avoid callbacks when killed 
                                      # for dynamic mngmt
        ...
        self.runner = None
        self.callbacks = set()
        self.basic_callbacks = set()
        self.success_callbacks = set()
        self._kill_pg = True
    ...
    def add_basic_callback(self, fn):
        self.basic_callbacks.add(fn)

    def add_success_callback(self, fn):
        self.success_callbacks.add(fn)
    ...
    def _run(self):
        ...
        with self._state_lock:
            if self._killed:
                ...
                print("Opening a new process\n")
                self._popen(args)

        if self._p is None:
            self._run_callbacks()
            return
        print("Output dir : " + self.working_dir);
        self.connection_path = _get_path(self.working_dir, str(CONNECTION_NAME) + "." + str(self._p.pid), None)
        self._run_success_callbacks()
    ...
    def _popen(self, args):
        ...
        env = os.environ.copy()
        env['LD_LIBRARY_PATH'] = '/Users/Swati/Documents/INSTALLS/lib'
...
class Pipeline(object):
    ...
    def to_String(self):
        # Pipeline Id 
        string = 'PIPELINE_ID: ' + str(self.id) + 'TOTAL_PROCS: ' + str(self.total_procs)  
        for run in self.runs:
            string += '\n Run Name: ' + run.name + 'NPROCS: ' + str(run.nprocs)  
        return string
    
    def to_json(self):
        # Pipeline Id 
        #j_string = '{ \"pipe_name\": \"' + str(self.id) + '\" , \"total_procs\": \"' + str(self.total_procs) + '\" , \"running_components\" : [' 
        j_string= {}
        j_string["pipe_name"] = str(self.id) 
        j_string["total_procs"] = str(self.total_procs)  
        comps = []
        for run in self._active_runs:
            comp = {}
            comp["comp_name"] = run.name 
            comp["procs_used"] = str(run.nprocs)  
            comp["args"] = str(run.args)  
            comps.append(comp)
 
        j_string["running_components"] = comps
        return j_string
    ...
    def start(self, consumer, runner=None):
        ...
        with self._state_lock:
            for run in self.runs:
                ...
                run.add_basic_callback(self.run_finished)
                run.add_success_callback(self.add_rprocessid)
    ...
    def add_rprocessid(self, run):
        with self._state_lock:
            self._active_pids[run.name] = run._p.pid
            self._active_connfiles[run.name] = run.connection_path

    def get_filename(self, runname):
        filename = ''
        with self._state_lock:
             filename = self._active_connfiles[runname]
        return filename

    def get_pid(self, runname):
        pid = -1
        with self._state_lock:
             pid = self._active_pids[runname]
        return pid
    ...
    def force_restart(self, consumer, params):
        ...
        new_procs = 0
        for run in self._active_runs:
            print("Checking " + run.name + "against " + name + "\n", flush = True)
            if run.name == str(name):
                print("Found run"+ str(run.tasks_per_node) + "\n", flush = True)
                if True: #run.tasks_per_node >= run.nprocs + 1:
                    run._p.poll()
                    assert run._p.returncode is None
                    ...
                    with self._state_lock:
                        ...
                        #assert new_run.nodes - run.nodes == 0

                    run.kill()

                    print("Killed the run" + run.name + "\n", flush = True ) 	 
                    run.join()
                      
                    print("Now starting a new job...\n", flush = True ) 	 
             
                #Do we need to change any parameters??
                    with self._state_lock:
