...
struct itimerval& TheTauInterruptInterval(void) { 
  interval.it_interval.tv_sec = 0;
  interval.it_interval.tv_usec = 0;
  interval.it_value = TauEnv_get_interval();
  return interval; 
}
...
void TauSetInterruptInterval(struct timeval timer) {
  /* Set the interval */
  //TheTauInterruptInterval() = interval;
  interval.it_interval.tv_sec = 0;
  interval.it_interval.tv_usec = 0;
  interval.it_value.tv_usec = timer.tv_usec;
  interval.it_value.tv_sec = timer.tv_sec;
  TheTauInterruptInterval() = interval;
}
...
void TauAlarmHandler(int signum) {
  ...
  setitimer(ITIMER_REAL, &(TheTauInterruptInterval()), NULL);
#endif
