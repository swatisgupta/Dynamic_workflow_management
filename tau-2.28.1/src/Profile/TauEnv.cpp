...
static struct timeval env_interval; 
...
struct timeval TauEnv_get_interval() {
  return env_interval;
}
...
void TauEnv_initialize() 
{
  ...
  if (!initialized) {
    ...
    const char *interval_e = getconf("TAU_INTERRUPT_INTERVAL");
    env_interval.tv_sec = TAU_INTERRUPT_INTERVAL_DEFAULT;
    env_interval.tv_usec = 0;
/**
    if (interval) {
...
**/
    ...
    if (interval_e) {
      int interval_value_sec = 0;
      int interval_value_usec = 0;
      if ( strchr ( interval_e, ':' ) != NULL) 
          sscanf(interval_e,"%d:%d", &interval_value_sec, &interval_value_usec);
      else
          sscanf(interval_e,"%d", &interval_value_sec);
      env_interval.tv_sec = interval_value_sec;
      env_interval.tv_usec = interval_value_usec;
      sprintf(tmpstr, "%d:%d", env_interval.tv_sec, env_interval.tv_usec);
      TAU_SET_INTERRUPT_INTERVAL(env_interval); 
      TAU_METADATA("TAU_INTERRUPT_INTERVAL", tmpstr);
    }
